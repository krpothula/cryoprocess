...
class CTFBuilder extends BaseJobBuilder {
   ...
   */
  get supportsGpu() {
    return getBoolParam(this.data, ['useGctf'], false);
  }
  ...
  buildCommand(outputDir, jobName) {
    ...
    const gpuEnabled = getBoolParam(data, ['useGctf'], false);
    ...
    if (gpuEnabled) {
      cmd.push('--use_gctf');
      const gctfExe = getParam(data, ['gctfExecutable'], settings.GCTF_EXE || '/usr/local/bin/gctf');
      if (!isPathSafe(gctfExe)) {
        throw new Error(`Invalid Gctf executable path: contains unsafe characters`);
      }
      cmd.push('--gctf_exe', gctfExe);
    } else {
      const ctffindExe = getParam(data, ['ctfFindExecutable', 'ctffindExecutable'], settings.CTFFIND_EXE || '/usr/local/bin/ctffind');
