...
const DefocusChart = ({ data = [], micrographs = [] }) => {
  ...
  const chartData = React.useMemo(() => {
    ...
    return micrographs.slice(-30).map((m, idx) => ({
      index: idx,
      defocus: m.defocusAvg,
      resolution: m.ctfMaxResolution,
      name: m.micrographName,
    }));
  }, [data, micrographs]);

  if (chartData.length === 0) {
    return (
      <div className="h-64 flex flex-col items-center justify-center text-[var(--color-text-muted)] bg-[var(--color-bg)] rounded-lg">
