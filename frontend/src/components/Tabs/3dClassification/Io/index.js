  ...
  return (
    <div className="tab-content">
      <CustomInput
        ...
        tooltipText="STAR file from Particle Extraction, 2D Classification, Subset Selection, or Initial Model"
        label="Input images STAR file:"
        value={formData?.["inputStarFile"]}
      />
      <CustomInput
        stageOptimiserFiles="Class3D"
        onChange={(val = "") => {
          handleInputChange({ target: { name: "continueFrom", value: val } });
        }}
        name="continueFrom"
        placeholder="Select optimiser file to continue a stalled job"
        tooltipText="Continue from a previous 3D Classification run. Select the latest _optimiser.star file from a stalled or incomplete job."
      ...
      <CustomInput
        ...
        tooltipText="Optional solvent mask"
